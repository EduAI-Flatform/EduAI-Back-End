...
`mode` is optional and defaults to `login`. Existing PostgreSQL roles are
preserved. A new Firebase user must provide `role`; otherwise the endpoint
returns `ACCOUNT_ROLE_REQUIRED` without creating an account.
...
Existing Google users can sign in immediately. New Google users must choose a
role before account creation. Password-provider users must have
`email_verified=true`; otherwise the endpoint returns `EMAIL_NOT_VERIFIED` and
does not issue an EduAI JWT session. New users receive the requested role and
never receive a password hash.
...
`INVALID_FIREBASE_TOKEN`, `ACCOUNT_BLOCKED`, `ACCOUNT_LINK_CONFLICT`,
`ACCOUNT_ROLE_REQUIRED`, and `FIREBASE_NOT_CONFIGURED`.
